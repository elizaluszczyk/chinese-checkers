...
public class ClientHandler implements Runnable {
  ...
  public void run() {
    try {
      ...
      logger.error(
          "Connection lost with client: {}", player != null ? player.getUsername() : "unknown", e);
  ...
  private void setupGameSettings(int currentPlayers)
      throws ClassNotFoundException, IOException, UnknownPacketException {
    if (currentPlayers == 1) {
      transmit(
          PacketType.REQUEST_GAME_SETTINGS,
          "Enter the number of players (2, 3, 4, or 6)",
          "Enter the game variant (default)");
      ServerPacket serverPacket = (ServerPacket) objectInputStream.readObject();
      handlePacket(serverPacket);
    } else {
  ...
  private void initializeGameManager() throws UnknownPacketException {
    GameManagerSingleton.setInstance(
        new StandardGameManager(GameServer.getGameType(), GameServer.getNumberOfPlayers()));
  ...
  private boolean validateNumberOfPlayers(String gameType, int numberOfPlayers)
    ...
    boolean isValid =
        switch (gameType) {
          case GAME_TYPE_DEFAULT ->
              numberOfPlayers >= 2 && numberOfPlayers != 5 && numberOfPlayers <= 6;
          case GAME_TYPE_DEFAULT_WITH_BOT ->
              numberOfPlayers >= 1 && numberOfPlayers != 4 && numberOfPlayers <= 5;
    ...
    ServerPacket packet =
        switch (type) {
          ...
          case REQUEST_GAME_SETTINGS ->
              new RequestGameSettingsPacket((String) args[0], (String) args[1]);
          ...
          case TURN_SKIP, USERNAME, GAME_SETTINGS ->
              throw new UnknownPacketException(
                  "Cannot transmit server-to-client packet type: " + type);
  ...
  private void handlePacket(ServerPacket packet) throws UnknownPacketException {
    switch (packet.getType()) {
      ...
      case BOARD_UPDATE,
              ...
              WIN ->
          throw new UnknownPacketException(
              "Unexpected client-to-server packet received: " + packet.getType());
  ...
  private void handleGameSettings(GameSettingsPacket packet) throws UnknownPacketException {
    ...
    transmit(
        ...
        "Game settings applied: " + numberOfPlayers + " players, variant: " + gameType);
  }
