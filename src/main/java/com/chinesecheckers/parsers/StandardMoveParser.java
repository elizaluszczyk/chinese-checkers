  ...
  public Move parseMove(String message) throws InvalidMoveException {
    ...
    if (!trimmedMessage.startsWith(MOVE_COMMAND)) {
      ...
      throw new InvalidMoveException(
          "Invalid move format. Move should start with '" + MOVE_COMMAND + "'.");
    }
    ...
    try {
      ...
      if (parts.length != 4) {
        logger.warn(
            "Invalid move format, incorrect number of parts ({}): {}",
            parts.length,
            trimmedMessage);
      ...
      if (!parts[2].equalsIgnoreCase(TO_KEYWORD)) {
        ...
        throw new InvalidMoveException(
            "Invalid move format. Expected keyword '" + TO_KEYWORD + "' as the third part.");
      }
      ...
      if (startPosition.length != 2 || endPosition.length != 2) {
        ...
        throw new InvalidMoveException(
            "Invalid coordinates format. Expected 'x,y' for both positions.");
      }
